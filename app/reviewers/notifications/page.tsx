...
import { useNotifications, type ReviewerNotificationType } from "@/lib/notification-context"
...
const notificationTypeLabels: Record<ReviewerNotificationType, string> = {
...
export default function ReviewerNotificationsPage() {
  ...
  const [selectedType, setSelectedType] = useState<ReviewerNotificationType | "all">("all")

  const reviewerNotifications = getNotificationsByRole("REVIEWER")

  const filteredNotifications = reviewerNotifications.filter((notification) => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-6 flex gap-2 border-b border-border">
          ...
          >
            Unread ({reviewerNotifications.filter((n) => !n.read).length})
          </button>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {Object.entries(notificationTypeLabels).map(([type, label]) => (
              <Button
                ...
                size="sm"
                onClick={() => setSelectedType(type as ReviewerNotificationType)}
              >
                {label}
              </Button>
            ))}
                  ...
                  <div className="flex-1">
                    <div className="mb-1 flex items-center gap-2">
                      ...
                      <span className="rounded-full bg-secondary px-2 py-0.5 text-xs text-muted-foreground">
                        {notificationTypeLabels[notification.type as ReviewerNotificationType]}
                      </span>
                    </div>
